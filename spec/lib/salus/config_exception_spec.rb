 ...
 describe '#initialize' do
  ...
  describe 'validation' do
    it 'should require the expected fields' do
      ...
      expect(Salus::ConfigException.new({'advisory_id' => 1, 'notes' => "", "changed_by"=> nil}).valid?).to be false
      expect(Salus::ConfigException.new({'advisory_id' => 1, 'notes' => "", "changed_by"=> ""}).valid?).to be true
    end
  end
