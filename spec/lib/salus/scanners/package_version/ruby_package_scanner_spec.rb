    ...
    context 'Gemfile.lock file present' do
      it 'should fail when package in repo does not fall within the specified package range' do
        ...
        expect(scanner.report.to_h).to include(
          { errors: [{
            message: "Package version for (actionmailer) (4.1.15) is greater"\
      ...
      it 'should fail when package in repo matched blocked range' do
        ...
        expect(scanner.report.to_h).to include(
          { errors: [{
            message: "Package version for (actionmailer) (4.1.15) matches"\
          " the configured blocked version (4.1.15) in Gemfile.lock."
          }] }
        )
      end
