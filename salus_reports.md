...
By default, Salus always prints the report to STDOUT. This can be turned off via the `--quiet` (`-q`) flag.
```sh Running Salus without printing report to STDOUT
 docker run --rm -t -v $(pwd):/home/repo coinbase/salus --quiet 
```

Additionally, Salus can be configured to send reports to both the local file system or remote HTTP endpoints. See the [configuration documentation](/configuration/salus_configurations) for more detail.  
