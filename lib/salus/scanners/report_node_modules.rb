...
module Salus::Scanners
  class ReportNodeModules < Base
    ...
    def record_dependencies_from_bower_json
      ...
      # Record if custom registry is defined 
      sources = {}
      sources['registry'] = '<package manager default>'
      if @repository.bowerrc_present?
        @repository.bowerrc.split("\n").each do |line|
          if line.include? "registry" 
            if URI.extract(line)
              sources['registry'] = URI.extract(line)
            end
          end
        end
      end
