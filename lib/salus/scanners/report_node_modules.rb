  ...
  class ReportNodeModules < Base
    ...
    def record_dependencies_from_latest_yarn_lock
      shell_return = run_shell(
        "yarn info --all --name-only --json", chdir: @repository.path_to_repo
      )
      ...
      source_shell_return = run_shell(
        "yarn config get npmRegistryServer", chdir: @repository.path_to_repo
      )
      unless source_shell_return.success?
        raise ParseError, "Failed to fetch npmRegistryServer: #{shell_return.stderr}"
      end

      dependencies = []
      source = source_shell_return.stdout
      yarn_info = shell_return.stdout
      yarn_info.each_line do |line|
        name = line.split("@npm")[0]
        version = line.split("@npm")[1]
        if name.present? && version.present?
          version = version.gsub("%3A", "")
          dependencies.append(
            {
