  ...
  class ReportNodeModules < ReportBase
    ...
    def record_dependencies_from_legacy_yarn_lock
      ...
      yarn_lock['object'].each do |dependency, info|
        ...
        url = get_distribution_url(package_info)
        record_node_module(
          name: name,
          version: version,
          source: source,
    ...
    def get_distribution_url(package_info)
      url = nil
      data = package_info["data"]
      has_dist = data["dist"] && data["dist"]["tarball"]
      has_repo_url = data["repository"] && data["repository"]["url"]
      url = data["repository"]["url"] if has_repo_url 
      url = data["dist"]["tarball"] if has_dist && url.nil?
      url
    end
    ...
    def record_node_module(dependency_file:, name:, version:, source:, external_references:, scope: SCOPE_REQUIRED)
      report_dependency(
        dependency_file,
        type: 'node_module',
        name: name,
