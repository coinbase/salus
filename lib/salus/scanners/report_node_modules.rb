  ...
  class ReportNodeModules < Base
    ...
    def record_dependencies_from_bower_json
      begin
        ...
        # Handle single registry -  {"registry": "https://registry.bower.io"}
        # Handle multiple registries - {"registry": {"search": ["http://localhost:9000","https://registry.bower.io"] }}
        sources = '<bower manager default>'
        if @repository.bowerrc_present?
          file = JSON.parse(@repository.bowerrc)
          if file.key?('registry')
            has_nested = file['registry'] && file['registry']['search']
            if has_nested.nil?
              sources = file['registry']
            else
              sources = file['registry']['search'].join(',')
            end
          end
        end
        ...
        error_msg = "Failed to parse JSON when recording bower dependencies."
