module Cyclonedx
  class ReportPythonModules < Base
    ...
    def version_string(dependency, is_purl_version = false)
      # If the dependency is specified in requirements.txt and an absolute version is needed for
      # the purl return empty
      return "" if dependency[:dependency_file] == 'requirements.txt' && is_purl_version
